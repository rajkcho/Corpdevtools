...
// Based on Constellation Software / Harris Computer methodology
// Sources: Mark Leonard shareholder letters, In Practise interviews
// with former CSU/Harris M&A professionals, analyst deep-dives
// ============================================================
...
const commercialTasks: TaskTemplate[] = [
  {
    ...
    children: [
      ...
      { title: 'Identify any well-funded horizontal competitors entering the vertical', priority: 'high', description: 'CSU red flag: new horizontal entrant with deep pockets disrupting niche' },
      { title: 'Evaluate platform dependency risk (e.g., built on Salesforce, AWS-specific)', priority: 'medium', description: 'CSU red flag: single platform vendor dependency' },
  ...
  },
  {
    ...
    children: [
      ...
      { title: 'Assess mission-criticality: can customers operate daily without the software?', priority: 'critical', description: 'CSU Tier 1 criteria: software must be mission-critical to daily operations' },
      { title: 'Evaluate switching costs — how deeply embedded is the product in customer workflows?', priority: 'critical', description: 'CSU loves businesses where "ripping it out would be incredibly painful"' },
...
const financialTasks: TaskTemplate[] = [
  {
    ...
    priority: 'critical',
    children: [
      { title: 'Break down revenue by type: recurring (SaaS/maintenance), license, professional services, other', priority: 'critical', description: 'CSU requires >60-70% recurring revenue. Below this threshold is typically a pass.' },
      { title: 'Calculate recurring revenue % of total revenue', priority: 'critical' },
      { title: 'Analyze maintenance/license revenue ratio (for on-prem: maintenance as multiple of initial license)', priority: 'high', description: 'CSU likes 3-5x+ lifetime maintenance vs. initial license fee' },
      { title: 'Assess professional services revenue quality — is it recurring or project-based?', priority: 'high', description: 'CSU red flag: revenue heavily weighted to PS/implementation' },
      { title: 'Review revenue recognition policies and deferred revenue trends', priority: 'high', description: 'CSU red flag: declining deferred revenue is a leading indicator of future revenue problems' },
  ...
  },
  {
    ...
    priority: 'critical',
    children: [
      { title: 'Normalize for owner compensation and personal expenses through P&L', priority: 'critical', description: 'CSU: "The first thing we do is normalize the financials. Most small VMS companies have owner personal expenses running through the P&L."' },
      ...
      { title: 'Normalize working capital (identify any customer prepayments masking churn)', priority: 'high', description: 'CSU red flag: multi-year deals that hide underlying attrition' },
    ],
  },
  {
    ...
    children: [
      ...
      { title: 'Calculate maintenance/recurring revenue margin', priority: 'critical', description: 'Often the single most important margin metric for CSU' },
      { title: 'Analyze cash conversion: EBITDA to free cash flow', priority: 'critical', description: 'CSU focuses on FCF, not accounting earnings' },
  ...
  },
  {
    ...
    priority: 'critical',
    children: [
      { title: 'Build 10-year DCF model with terminal value (perpetuity growth method)', priority: 'critical', description: 'CSU standard valuation methodology' },
      { title: 'Model probability-weighted scenarios (MECE: bull/base/bear)', priority: 'critical', description: 'CSU uses MECE scenario modeling to produce single hurdle rate across different risk profiles' },
      { title: 'Calculate IRR against Harris hurdle rates (<$1M=30%, $1-4M=25%, >$4M=20%)', priority: 'critical' },
      { title: 'Calculate revenue multiple and EBITDA multiple (CSU typically pays 0.5-1.5x revenue or 4-8x EBITDA)', priority: 'high' },
...
const customerTasks: TaskTemplate[] = [
  {
    ...
    children: [
      ...
      { title: 'Calculate customer concentration: Top 10 customers as % of revenue', priority: 'critical', description: 'CSU red flag: any single customer >15-20% of revenue. Some sources say >10%.' },
      { title: 'Calculate Herfindahl-Hirschman Index (HHI) for revenue concentration', priority: 'high', description: 'CSU computes HHI or equivalent concentration measure' },
      { title: 'Assess industry concentration even if no single customer is large', priority: 'high', description: 'CSU: if all customers are in one sub-segment, that\'s concentration risk' },
      ...
      { title: 'Assess government/single-payer dependency', priority: 'high', description: 'CSU red flag: budget risk, procurement cycle risk' },
    ],
  },
  {
    ...
    priority: 'critical',
    children: [
      { title: 'Calculate annual logo churn (gross attrition) by segment', priority: 'critical', description: 'CSU target: <10% annual logo churn' },
      { title: 'Calculate dollar churn (revenue-weighted attrition)', priority: 'critical' },
      { title: 'Calculate Gross Revenue Retention (GRR)', priority: 'critical', description: 'CSU target: >90%. Below 85% is a significant concern.' },
      { title: 'Calculate Net Revenue Retention (NRR)', priority: 'high' },
      { title: 'Build customer cohort retention curves by vintage year', priority: 'high', description: 'CSU analyzes revenue per customer segmented by cohort/vintage year' },
      { title: 'Identify customers on legacy contracts with below-market pricing', priority: 'high', description: 'CSU red flag: risk of sticker shock on renewal' },
      { title: 'Review contract renewal dates — flag clusters in first 12 months post-acquisition', priority: 'high', description: 'CSU: cluster of renewals in year 1 post-close is a risk' },
  ...
  },
  {
    ...
    priority: 'critical',
    children: [
      { title: 'Select 10-20 customers for reference calls (WE pick, not the seller)', priority: 'critical', description: 'CSU: "We always do customer calls. Minimum 5-10 reference calls, and we pick the customers, not the seller."' },
      ...
      { title: 'Flag any "stuck not happy" customers — high switching costs but low satisfaction', priority: 'critical', description: 'CSU red flag: switching costs without satisfaction = vulnerability to disruption' },
    ],
  },
];
...
const technologyTasks: TaskTemplate[] = [
  {
    ...
    children: [
      ...
      { title: 'Assess: can this platform be maintained for 10+ years without a rewrite?', priority: 'critical', description: 'CSU: "The tech DD was not about whether the code was beautiful. It was about: can we maintain this for 10+ years without a rewrite?"' },
      { title: 'Evaluate tech debt level and modernization cost estimate', priority: 'critical', description: 'CSU: tech debt should be priced into the deal. Not "old tech" but "unmaintainable tech" is the red flag.' },
      { title: 'Review CI/CD pipeline and automated testing coverage', priority: 'high', description: 'CSU red flag: no automated testing / CI/CD indicates fragile codebase' },
      { title: 'Assess single-developer dependency on codebase', priority: 'critical', description: 'CSU red flag: one person who understands the codebase' },
      { title: 'Review code repository history and contribution patterns', priority: 'medium' },
      { title: 'Identify any "ticking clock" — end-of-life framework, database, or OS', priority: 'high', description: 'CSU: is there a ticking clock requiring expensive, risky migration?' },
    ],
  },
  {
    ...
    priority: 'high',
    children: [
      { title: 'Review security posture and any recent vulnerability assessments', priority: 'high', description: 'CSU: especially critical in regulated verticals' },
      { title: 'Assess data privacy compliance (GDPR, HIPAA, SOC2 as applicable)', priority: 'high' },
      { title: 'Review open-source licensing for compliance risk', priority: 'high', description: 'CSU red flag: significant open-source licensing risk' },
...
const hrTasks: TaskTemplate[] = [
  {
    ...
    priority: 'critical',
    children: [
      { title: 'Map ALL critical knowledge holders: codebase, customer relationships, operations', priority: 'critical', description: 'CSU maps all critical knowledge holders during DD' },
      { title: 'Assess founder dependency — is the founder the sole key relationship holder with major customers?', priority: 'critical', description: 'CSU red flag: founder IS the product (consulting practice wrapped in software) = pass' },
      { title: 'Evaluate depth of management bench — is there a capable #2?', priority: 'critical', description: 'CSU: "bus factor" of 1 is a key risk' },
      { title: 'Assess whether critical knowledge is documented or tacit', priority: 'high', description: 'CSU plans for knowledge transfer during integration' },
      { title: 'Identify founder/owner retention plan: earnout, retention agreement (1-3 years)', priority: 'high', description: 'CSU structures earnouts and retention agreements for founder-dependent businesses' },
    ],
  },
  {
    ...
    children: [
      ...
      { title: 'Review all employee compensation vs. market benchmarks', priority: 'high', description: 'CSU red flag: compensation significantly below market = retention risk post-acquisition' },
      ...
      { title: 'Calculate revenue per employee and EBITDA per employee', priority: 'high', description: 'CSU benchmarks: Revenue/employee $100K-$200K target' },
    ],
  },
  {
    ...
    priority: 'medium',
    children: [
      { title: 'Assess cultural fit with CSU/Harris operating model (measurement, reporting, accountability)', priority: 'high', description: 'CSU red flag: teams that resist measurement, reporting, accountability' },
...
const operationalTasks: TaskTemplate[] = [
  {
    ...
    priority: 'critical',
    children: [
      { title: 'Calculate R&D as % of revenue (CSU range: 15-25% at acquisition, target 12-20%)', priority: 'critical', description: 'CSU benchmarks every acquired company against its peer group' },
      { title: 'Calculate S&M as % of revenue (CSU range: 10-20% at acquisition, target 8-15%)', priority: 'critical' },
      { title: 'Calculate G&A as % of revenue (CSU range: 10-20% at acquisition, target 8-12%)', priority: 'critical' },
      { title: 'Calculate Support/COGS as % of revenue (CSU range: 15-25% at acquisition, target 12-20%)', priority: 'high' },
      { title: 'Calculate maintenance/recurring revenue margin (CSU target: 80-90%)', priority: 'critical' },
      { title: 'Calculate cash conversion: FCF/EBITDA (CSU target: 80-95%)', priority: 'high' },
      { title: 'Compare all ratios against Harris/CSU portfolio benchmarks', priority: 'high', description: 'CSU: "If your R&D spend is 25% and the peer group is at 18%, we\'ll ask why."' },
    ],
  },
  {
    ...
    priority: 'medium',
    children: [
      { title: 'Review customer support operations: ticket volumes, resolution times, SLA compliance', priority: 'high', description: 'CSU DD workstream: support ticket analysis, SLA compliance' },
  ...
  },
  {
    ...
    priority: 'high',
    children: [
      { title: 'Develop Day-1 readiness plan', priority: 'high', description: 'CSU: "The first thing we do is implement our reporting within 30 days of close."' },
      { title: 'Identify quick wins: pricing adjustments, cost rationalization opportunities', priority: 'high', description: 'CSU 100-day review: execute quick wins' },
      ...
      { title: 'Identify shared services opportunities (finance, HR, IT)', priority: 'medium', description: 'CSU optimizes G&A through shared services' },
